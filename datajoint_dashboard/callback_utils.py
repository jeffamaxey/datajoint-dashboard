...
def clean_single_gui_record(d, attrs, master_key=None, add_master_key=False):
    ...
    if (master_key or set(d.values()) == {''}) and (
        not master_key
        or {
            v
            for k, v in d.items()
    ...
    if not master_key or not add_master_key:
        return d
...
def clean_gui_data(table, data, master_key=None, add_master_key=False):
    ...
    clean_data = []
    for d in data:
        if clean_d := clean_single_gui_record(
...
def insert_part_table(part_table, master_key, new_data, msg=''):
    ...
    try:
        ...
        msg = (
            f'{msg}Successfully inserted records into part table '
        ...
        for r in new_data:
            try:
                ...
                msg = (
                    f'{msg}Successfully inserted record in part table '
                    + f'{part_table.__name__}, {r}:.\n'
                )
            except Exception as e:
                msg = (
                    f'{msg}Error inserting record in part table '
    ...
    for f in table.heading.secondary_attributes:
        if new[f] != old[f]:
            try:
                ...
                msg = f'{msg}Successfully updated field {f} ' + f'from {old[f]} to {new[f]}!\n'
            except Exception as e:
                msg = msg + str(e) + '\n'
...
def update_part_table(part_table, master_key, new_data, msg=''):
    ...
    if new_data == old_data:
        return f'{msg}Part table {part_table.__name__} data unchanged.\n'
    new_data_pk = [
    ...
    if pks_to_be_deleted := [
        pk for pk in old_data_pk if pk not in new_data_pk
    ]:
        try:
            (part_table & pks_to_be_deleted).delete(force=True)
            msg = (
                f'{msg}Successfully deleted records from part table '
                + f'{part_table.__name__}.\n'
            )
        except Exception as e:
            msg = (
                f'{msg}Error deleting part table '
                + f'{part_table.__name__}.: {str(e)}\n'
            )

    if new_records := [
        d for pk, d in zip(new_data_pk, new_data) if pk not in old_data_pk
    ]:
        try:
    ...
    if records_to_be_updated := [
        (pk, d)
        for pk, d in zip(new_data_pk, new_data)
            ...
            for (old_k, old_v), (new_k, new_v) in zip(old_record.items(), new_record.items()):
                ...
                if old_k not in pks and old_v != new_v:
                    try:
                        ...
                        msg = (
                            f'{msg}Successful update in part table '
                        ...
                        msg = (
                            f'{msg}Error when updating part table '
