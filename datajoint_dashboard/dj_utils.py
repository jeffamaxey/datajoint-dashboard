    ...
    for key, value in foreign_keys.items():
        ...
        if field in dependent_fields:
            try:
                ...
                parent_field = list(value['attr_map'].values())[dependent_fields.index(field)]
            break
...
def get_default(query, field):
    ...
    '''
    if not (default := query.heading.attributes[field].default):
        return ''
    if matches := re.findall(
        r'\"(.+?)\"', query.heading.attributes[field].default
    ):
        return (
